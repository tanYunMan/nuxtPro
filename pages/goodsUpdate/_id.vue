...
export default {
  ...
  methods: {
    getGoodsById(id) {
      this.$axios.get('/goods/goods/getAdminGoods', {params: {goodsID: id}}).then(res=>{
        if(res.code === 1001) {
          this.goods = res.body
          if (this.goods.isBlod === 1) {
            this.$set(this.goods, 'blod', true)
          } else {
            this.$set(this.goods, 'blod', false)
          }
        }
      })
    },
    update() {
      this.$refs['goods'].validate((valid) => {
        if (valid) {
          ...
          this.$axios
            .post('/goods/goods/updateGoods', null, {
              params: this.goods
            })
            .then((res) => {
