<template>
  <div>
    ...
    <section class="container container-good">
      <div class="filter-wrap clear-top">
        <el-form :model="queryParams" ref="form" label-width="80px" :inline="true" size="small">
          <el-form-item>
          ...
          <el-form-item>
            <el-input v-model="queryParams.typeValue" placeholder="请输入关键字"></el-input>
          </el-form-item>
          <el-form-item>
            <el-input v-model="queryParams.goodsTypeID" placeholder="请输入商品类型id"></el-input>
          </el-form-item>
          <el-form-item>
      ...
      >
        ...
        >
          <template slot-scope="scope">
            ...
            <div v-if="scope.row.goodsName.length < 20">
              <div class="go-btn" :class="{'no':!scope.row.agentType}">供</div>
              <span
                :style="{'color':scope.row.color}"
                :class="{'font-blod':scope.row.isBlod == 1}"
        ...
        >
          <template slot-scope="scope">
            <el-button
              size="mini"
              type="text"
              @click="updateStates(scope.row)"
            >{{scope.row.goodsState | filterState}}</el-button>
          </template>
        </el-table-column>
        <el-table-column
          :show-overflow-tooltip="true"
        ...
        >
          <template slot-scope="scope">
            <el-popover placement="top-start" title="本地目录--名称" width="500" trigger="hover">
              <p v-if="scope.row.catalog">{{ scope.row.catalog.catalogName }}</p>
              <el-button
                slot="reference"
                class="el-hidden-tc"
                v-if="scope.row.catalog"
              >{{scope.row.catalog.catalogName}}</el-button>
            </el-popover>
          </template>
        </el-table-column>
        <el-table-column
          ...
          >
            ...
            <el-form-item label="点卡内容：" prop="textarea">
              <el-input
                ...
                :rows="4"
                placeholder="请输入点卡内容"
                v-model="importCardForm.textarea"
              ></el-input>
            ...
            <el-form-item label="该卡截止有效日期：">
              <el-date-picker
                v-model="importCardForm.validDate"
                type="date"
                placeholder="选择日期"
            ...
            <el-form-item label="判断（库存）重复：">
              <el-switch
                v-model="importCardForm.isRepeatState"
                active-color="#13ce66"
                inactive-color="#eee"
              ></el-switch>
            </el-form-item>
            <el-form-item label="是否优先销售：">
              <el-switch
                v-model="importCardForm.isFirstState"
                active-color="#13ce66"
                inactive-color="#eee"
              ></el-switch>
            </el-form-item>
      ...
      <el-dialog title="库存查看" :visible.sync="dialog4" width="70%">
        <div class="filter-wrap">
          <el-form :model="kcQueryParams" label-width="80px" :inline="true" size="small">
            <el-form-item label="卡号">
              <el-input placeholder="请输入要查找的卡号" v-model="kcQueryParams.cardNumber"></el-input>
            </el-form-item>
            <el-form-item>
      ...
      >
        <div class="item1 item">
          ...
          <div class="label">
            <span>等级价格组：</span>
            <el-select
              v-model="userLevelListID"
              placeholder="请选择等级价格组"
              @change="changeLevelListID"
              style="width:200px"
              clearable
              @clear="setValueNull(1)"
            >
              <el-option
          ...
          >
            ...
            <el-table-column :show-overflow-tooltip="true" prop="price" label="销售价" min-width="20%">
              <template slot-scope="scope">
                <span
                  v-if="scope.row.countPrice && scope.row.countPrice>0"
                  style="margin-left:15px;"
                >{{scope.row.countPrice}}元</span>
            ...
            <el-table-column :show-overflow-tooltip="true" prop="label3" label="操作" min-width="60%">
              <template slot-scope="scope">
                ...
                <el-input-number
                  @change="handleChangeInputNumber(scope.row)"
          ...
          <el-table border row-key="key" :data="tableData6" style="width: 100%;" class="table">
            ...
            <el-table-column :show-overflow-tooltip="true" prop="label2" label min-width="60%">
              <template slot-scope="scope">
                ...
                <el-input-number
                  v-if="scope.row.label2.id == 1"
                  v-model="scope.row.label2.value"
                  style="width:80%"
                  controls-position="right"
                  :precision="2"
                  :min="0"
      ...
      <el-dialog title="商品操作日志" :visible.sync="dialog6" width="70%" class="dialog6">
        <div class="filter-wrap">
          <span>详情关键字：</span>
          <el-input placeholder="请输入详情关键字："></el-input>
      ...
      <el-dialog title="选择商品在目录下展示" :visible.sync="dialog7" width="70%" class="dialog7">
        <div class="item" v-for="(item,index) in catalogTree" :key="index">
          ...
          <el-checkbox-group v-model="checkList" v-if="item.children" @change="checkCatalogID">
            <el-checkbox
              :label="item1.catalogID"
              :min="0"
              :max="1"
              v-for="(item1,index1) in item.children"
              :key="item1.catalogID"
            >{{item1.catalogName}}</el-checkbox>
          </el-checkbox-group>
        </div>
        <span slot="footer" class="dialog-footer">
      ...
      <el-dialog title="批量修改商品名称颜色" :visible.sync="dialogColor" width="60%" class="dialog8">
        ...
        <span slot="footer" class="dialog-footer">
          <el-button size="small" @click="dialogColor = false">取 消</el-button>
          <el-button
            size="small"
            type="primary"
            @click="confirmColor('formColor')"
            :loading="loading"
          >确 认 修 改</el-button>
        </span>
      </el-dialog>
...
export default {
  ...
  data() {
    return {
      ...
      importCardRules: {
        textarea: [
          { required: true, message: '请输入点卡内容', trigger: 'blur' }
        ]
      },
      dialog1: false,
      dialog2: false,
  ...
  methods: {
      ...
      newLevelPrice.map((item) => {
        newTableData5.map((items) => {
          if (item.levelID === items.levelID) {
            ...
            items.countPrice =
              Math.round(
                (this.goodsPrice + item.levelPrice) * Math.pow(10, 2)
              ) / Math.pow(10, 2)
          }
        })
      })
    ...
    getGoodsList() {
      this.loading = true
      this.$axios
        .post('/goods/goods/goodsPage', null, { params: this.queryParams })
        .then((res) => {
          if (res.code === 1001) {
    ...
    confirmSetCatalog() {
      ...
      this.$axios
        .post('/goods/goods/updateCatalogIDBatch', null, {
          params: {
            ids: this.delIds.toString(),
            catalogID: this.checkList.toString()
          }
        })
        .then((res) => {
          if (res.code === 1001) {
    ...
    delGoods(id) {
      this.$confirm('此操作将删除该自营商品, 是否继续?', '提示', {
        ...
        .then(() => {
          this.$axios
            .get('/goods/goods/deleteGoods', { params: { id: id } })
            .then((res) => {
              if (res.code === 1001) {
                this.$message({
                  showClose: true,
    ...
    confirmDel() {
      ...
      this.$axios
        .get(url, {
          params: { id: this.delIds.toString() }
        })
        .then((res) => {
          if (res.code === 1001) {
            this.$message({
    ...
    // 跳转编辑页面
    goDetail(id) {
      this.$router.push({ path: `/goodsUpdate/${id}` })
    },
    ...
    handleChangeInputNumber(value) {
      ...
      newTableData5.map((item) => {
        if (item.levelID === value.levelID) {
          item.countPrice =
            Math.round((this.goodsPrice + value.prices) * Math.pow(10, 2)) /
            Math.pow(10, 2)
        }
      })
    },
      ...
      if (thisval === 0) {
        ...
        if (data === 0) {
          ...
          this.tableData5.map((item, index) => {
            item.prices =
              thisval + thisval * (this.tableData5.length - (index + 1))
            //item.countPrice = this.goodsPrice + (thisval+thisval*index)
            item.countPrice =
              Math.round(
                (this.goodsPrice +
                  (thisval +
                    thisval * (this.tableData5.length - (index + 1)))) *
                  Math.pow(10, 2)
              ) / Math.pow(10, 2)
          })
        } else {
          ...
          this.tableData5.map((item, index) => {
            item.prices =
              this.goodsPrice *
              (thisval / 100 +
                (thisval / 100) * (this.tableData5.length - (index + 1)))
            item.countPrice =
              Math.round(
                (this.goodsPrice +
                  (thisval / 100 +
    ...
    //     this.supplyPrice = ''
    //   }
    //   this.$axios.get('/goods/goods/getGoodsPrice',null,{params: { goodsID: data.goodsID }}).then((res) => {
    //     if (res.code === 1001) {
    ...
    //           item.countPrice =
    //             Math.round((item.price + data.goodsPrice) * Math.pow(10, 2)) /
    //             Math.pow(10, 2)
    //           //}
    //         })
    ...
    getPriceGroupList() {
      this.$axios
        .post('/site/levelPriceGroup/page')
        .then((res) => {
          if (res.code === 1001) {
    ...
    goodsPriceInput(data) {
      if (this.tableData5.length === 0) {
        ...
        for (let i = 0; i < this.tableData5.length; i++) {
          this.tableData5[i].prices = this.tableData5[i].prices
            ? this.tableData5[i].prices
            : 0
          this.tableData5[i].price =
            Math.round((data + this.tableData5[i].prices) * Math.pow(10, 2)) /
            Math.pow(10, 2)
          this.tableData5[i].countPrice =
            Math.round((data + this.tableData5[i].prices) * Math.pow(10, 2)) /
            Math.pow(10, 2)
        }
      }
    ...
    goBuyingPrice() {
      ...
      for (let i = 0; i < this.tableData5.length; i++) {
        this.tableData5[i].price = this.tableData5[i].price
          ? this.tableData5[i].price
          : 0
        this.tableData5[i].prices = this.tableData5[i].prices
          ? Number(this.tableData5[i].prices)
          : 0
      ...
      this.$axios.post('/goods/goods/saveGoodsPrice', thisData).then((res) => {
        if (res.code === 1001) {
          this.$message({
            showClose: true,
            message: '定价成功',
    ...
    confirmColor(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if (ids) {
            this.$axios
              .post('/goods/goods/updateColorBatch', null, {
                params: { ...this.formColor, ids: ids.toString() }
              })
              .then((res) => {
                if (res.code === 1001) {
                  this.$message({
    ...
    updateState(goodsState) {
      ...
      if (goodsState === 2) {
        ...
        this.$axios
          .post('/goods/goods/updateGoodsStateBatch', null, {
            params: {
              ids: ids.toString(),
    ...
    confirm2() {
      ...
      if (this.goodsState == 2) {
        confirmForm = {
          ids: this.goodsIDState.toString(),
    ...
    getCatalogTree() {
      let arr = []
      this.$axios.get('/goods/catalog/tree').then((res) => {
    ...
    confirmImport() {
      ...
      this.$axios
        .post('/goods/goods/importCard', this.importCardForm, {
    ...
    getCardList() {
      this.$axios
        .post('/goods/card/page', null, { params: this.kcQueryParams })
        .then((res) => {
